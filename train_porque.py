...
from utils.dataloader import SmilesJEPADataset
...
def generation_eval(list_of_smiles, model, tokenizer, max_len):
    """
    ...
    with torch.no_grad():
        with torch.amp.autocast('cuda', dtype=torch.bfloat16):
            # Encode and get CLS token embedding
            encode_out = model.encoder(
                input_ids=input_ids,
                attention_mask=attention_mask,
            )
            latents = encode_out['hidden_state'][:, 0, :]  # CLS token
            generated_ids = model.decode(latent=latents.unsqueeze(1), temperature=0.0)
...
def do_evals_in_train_loop(smiles_to_use, model, tokenizer, max_len, device):
    """Run autoencoder reconstruction evaluation."""
    model.eval()
...
def main():
    ...
    while global_step < args.max_steps:
        ...
        for grad_accum_step in range(args.grad_accum_steps):
            ...
            view1_ids = batch['view1_ids'].to(device)
            view1_attention_mask = batch['view1_attention_mask'].to(device)
            view2_ids = batch['view2_ids'].to(device)
            ...
            with torch.autocast('cuda', dtype=torch.bfloat16):
                ...
                step_loss = decoder_loss * args.decode_loss_scale + contrastive_loss * args.contrastive_loss_scale + jepa_loss * args.jepa_loss_scale
            ...
            total_jepa_loss = total_jepa_loss + jepa_loss.detach() / args.grad_accum_steps
        ...
        if rank == 0:
            ...
            log_str = (
                ...
                f"JEPA: {total_jepa_loss.item():.4f} | "
            ...
            log_args = {
                ...
                "jepa_loss": total_jepa_loss.item(),
